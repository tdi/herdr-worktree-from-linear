...
export function issuePaneOpenArgs(pluginId, rootPaneId, identifier) {
  return [
...
export function openIssuePane(worktreeStdout, identifier, pluginId, exec = runCmd, herdrBin = 'herdr') {
  const rootPane = parseRootPaneId(worktreeStdout);
  if (!rootPane || !identifier || !pluginId) return null;
  const res = exec(herdrBin, issuePaneOpenArgs(pluginId, rootPane, identifier));
