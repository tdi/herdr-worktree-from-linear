...
export async function run({ env = process.env, exec = runCmd, fetchFn = fetch, select = defaultSelect, writeTicket = writeTicketConfig, log = (m) => process.stdout.write(`${m}\n`) } = {}) {
  ...
  try {
    const ticket = await fetchIssue(config, issue.identifier, fetchFn);
    const ticketPath = writeTicket(res.worktreePath, config.ticketConfigPath, ticket);
