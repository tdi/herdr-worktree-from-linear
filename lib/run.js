...
export async function run({ env = process.env, exec = runCmd, fetchFn = fetch, select = defaultSelect, log = (m) => process.stdout.write(`${m}\n`) } = {}) {
  ...
  const config = loadConfig(env.HERDR_PLUGIN_CONFIG_DIR, repoRoot);
  ...
  if (config.showIssueDetails && !res.exists) openIssuePane(res.stdout, issue.identifier, pluginId, exec, herdrBin, repoRoot);
